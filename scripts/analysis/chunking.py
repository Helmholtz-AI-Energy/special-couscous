...
if __name__ == "__main__":
    ...
    results_df = pd.DataFrame(
        [
            (k[0], k[1], k[2], k[3], v[0], v[1], v[2], v[3], v[4])
    ...
    overall_energy_strong_scaling_eq = results_df[results_df["Data seed"] == 0][
        "Energy consumed"
    ].sum()

    print(
        f"Overall energy consumed: {overall_energy} Wh"
        f"\nStrong scaling equivalent: {overall_energy_strong_scaling_eq} Wh"
    )
    ...
    print("Global test accuracy error:\n", std_global_test_accuracy)
    print("Speedup:\n", speedup)
    print("Speedup error:\n", std_speedup)
    ...
    # Save the figure.
    plt.savefig(pathlib.Path(root_dir) / f"{data_set}_chunking.pdf")
