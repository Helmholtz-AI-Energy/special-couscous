...
def load_and_verify_dataset(
    ...
    if expected_dataset_config != actual_dataset_config:
        ...
        missing_keys_hdf5 = [
            key for key in expected_dataset_config if key not in actual_dataset_config
        ]
        additional_keys_hdf5 = [
            key for key in actual_dataset_config if key not in expected_dataset_config
        ]
        keys_with_mismatched_values = [
            key
            for key, value in expected_dataset_config.items()
        ...
        if keys_with_mismatched_values:
            log.debug("Mismatched values:")
        for key in keys_with_mismatched_values:
            log.debug(
                f"{key}: expected {expected_dataset_config[key]} but got {actual_dataset_config[key]}"
            )
